...
'''
    sh "php artisan dotenv:set-key APP_URL ${url}"
}

def copyThemeToPackages() {
    copyArtifacts([
        projectName: 'codex/workflow/build-theme',
        filter     : 'theme.tar.gz',
        target     : 'theme',
...
def laravelPublishAssets() {
    sh 'rm -rf public/vendor'
    sh 'php artisan vendor:publish --force --tag=public -vvv'
}

def laravelStorageLink() {
    sh 'rm -rf public/vendor'
    sh 'php artisan vendor:publish --force --tag=public -vvv'
}

def _withCredentials(Closure body) {
