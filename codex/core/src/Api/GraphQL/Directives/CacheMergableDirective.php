...
class CacheMergableDirective extends BaseDirective implements FieldMiddleware
{
    /**
    ...
    public function handleField(FieldValue $value, \Closure $next)
    {
        ...
        return $value->setResolver(function ($root, $args, Context $context, $info) use ($value, $resolver, $maxAge, $privateCache) {
            ...
            if ($cacheHas) {
                $context->request->isMethodCacheable();
                return $useTags
            ...
            if ($resolvedValue instanceof Model) {
                $model         = $resolvedValue;
                $resolvedValue = $model->setHidden([])->attributesToArray();
                $model->rehide();
            }
